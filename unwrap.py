...
if __name__ == '__main__':
    shape = {"tag": "label", "shape": [{"x": 0.032232142857142842, "y": 0.03219140625},
                                      {"x": 0.50655701811449864, "y": 0.014404355243445227},
                                      {"x": 0.9732539682539681, "y": 0.03171875},
                                      {"x": 0.9766567460317459, "y": 0.9776953125},
                                      {"x": 0.48447501824501454, "y": 0.9952298867391453},
                                      {"x": 0.033134920634920626, "y": 0.97258984375}]}
    ...
    imcv = cv2.imread('image554.jpg', cv2.IMREAD_UNCHANGED)
    box= cv2.selectROI(imcv)
    # print(box)
    imcv=imcv[box[1]:box[3],box[0]:box[2]]
    padimage= cv2.copyMakeBorder(imcv, 0, 0, 20, 20, cv2.BORDER_CONSTANT, None, value = 0)
    ...
    sharpen = cv2.filter2D(dst_image, -1, sharpen_kernel)

    cv2.imshow('sharpen', sharpen)

    # cv2.imshow("image_with_mask.png", padimage)
    # cv2.imshow("unwrapped.jpg", dst_image)
